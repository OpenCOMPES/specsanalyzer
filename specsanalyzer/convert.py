...
def zinner(ek, angle, dapolymatrix):
    ...
    for i in range(0, dapolymatrix.shape[0]):
        # igor uses the fit poly 3, which should be a parabola
        result = (result + 
        10**(-(2*i))*angle**(1+2*i)*np.polyval(dapolymatrix[i][:], ek))
...
def zinner_diff(ek, angle, dapolymatrix):
    ...
    for i in range(0, dapolymatrix.shape[0]):
        # igor uses the fit poly 3, which should be a parabola
        result = result + (2*i+1)*10**(-(2*i))*angle**(2*i) * \
...
def mcp_position_mm(ek, angle, ainner, dapolymatrix):
    ...
    result = np.where(mask, zinner(ek, angle, dapolymatrix), np.sign(
        angle)*zinner(ek, angle, dapolymatrix)+(abs(angle)-ainner)*zinner_diff(ek, angle, dapolymatrix))
